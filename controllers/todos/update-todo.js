...
module.exports = async (req, res) => {
    try {
        const { accountId, todoId, content, completed, priority } = req.body || {};

        if (!accountId || !todoId || !content || !priority) {
        ...
        return response(res, 200, {
            success: true,
            message: "Thành công cập nhật công việc!"
        });
    }
    catch(error) {
        console.log(error);
